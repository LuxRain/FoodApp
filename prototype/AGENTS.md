...
- The approved visual direction is Option 1: a camera-first intake experience using the current iOS 26 Liquid Glass language.
- Reserve translucent glass for functional navigation, scan controls, sheets, and primary app chrome; keep data-heavy inventory content more opaque and readable.
- Use forest green as the primary action and acceptance color. Expired items are red, items within 1–2 weeks are yellow, and items beyond that window are green.
- Regular users scan and verify automatically filled fields, with manual edits as a fallback. Admins handle low-confidence exceptions and acceptance decisions.
- The donation dashboard is expiration-first and records quantity, date, calories when applicable, storage location, and allergen information.
